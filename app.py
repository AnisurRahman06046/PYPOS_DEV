...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # Creating the navigation buttons
        self.nav_buttons = []  # List to keep track of buttons
        nav_button_names = ["Home", "Orders", "POS Terminal", "Products", "Customers", "Staff", "Settings"]

        btn_style = "padding:10px;font-size:25px;border:none;color:black;"
        active_btn_style = "padding:10px;font-size:25px;border:none;color:blue;"  # Style for the active button

        for name in nav_button_names:
            button = QPushButton(name)
            button.setStyleSheet(btn_style)
            button.clicked.connect(lambda _, b=button: self.highlight_button(b, btn_style, active_btn_style))
            self.nav_buttons.append(button)
            left_nav.addWidget(button)
        ...
        self.prflBtn = QPushButton()
        load_prflLogo = resource_path("assets/profile.png")
        profilePix = QPixmap(load_prflLogo).scaled(40, 40, Qt.AspectRatioMode.KeepAspectRatio, Qt.TransformationMode.SmoothTransformation)
        self.prflBtn.setIcon(QIcon(profilePix))
        self.prflBtn.setIconSize(profilePix.size())
        self.prflBtn.setStyleSheet(btn_style)
        right_nav.addWidget(self.prflBtn)
        ...
        pagesTextList = ["Home Page", "Orders Page", "POS Terminal Page", "Products Page", "Customers Page", "Staff Page", "Settings Page"]

        for text in pagesTextList:
            self.pages.addWidget(self.create_page(text))
        ...
        # Highlight the default button (Home)
        self.highlight_button(self.nav_buttons[0], btn_style, active_btn_style)

    def highlight_button(self, clicked_button, default_style, active_style):
