...
from PyQt6.QtWidgets import (
    ...
    QStackedWidget, QLineEdit, QMessageBox, QSpacerItem, QSizePolicy
)
...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        # Define the navigation bar and main content layout here
        ...
        user = get_user_data()
        if user:
            user_name = f"{user[1]} {user[2]}"  # Assuming first_name and last_name are at index 1 and 2
        else:
            user_name = "Guest"
    ...
    def show_main_window(self):
        self.init_ui()  # Reinitialize the main UI
        self.show()  # Ensure the main window is displayed
    ...
    def logout(self):
        ...
        if confirm == QMessageBox.StandardButton.Yes:
            clear_user_data()  # Clear user data from the database
            self.show_login_page()  # Reset to login page
